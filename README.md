...
1. Open `CompetitiveHelper.py` and set the port:

```python
PORT = 10045
```

2. Open the **Competitive Companion** browser extension → Settings → Custom Port → set `10045`.
...
2. Search for `Competitive Helper: Start Listener` and select it.
3. The plugin will now listen for problems from Competitive Companion.

---

### 5. Fetch Problem Data

1. Open a competitive programming problem in your browser (e.g., Codeforces, AtCoder, HackerEarth).
2. Click the **Competitive Companion** extension icon.
3. The plugin automatically writes:
...
1. Start listener in Sublime.
2. Open a problem in the browser → click Competitive Companion.
3. Open `solve.cpp` → start coding with test cases preloaded from `input.txt`.
4. Run your solution and compare with `e-output.txt`.
...
* **SyntaxError**
  ...
  * Ensure you are using Sublime Text 4 (Python 3.3). The plugin uses `.format()` instead of f-strings.
